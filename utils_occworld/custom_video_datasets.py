...
class VideoNuscenesDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.multi_view:
            ...
            example = dict()
            example["pixel_values"] = torch.stack(pixel_values_list)
            # example["pixel_values_low"] = frames_low
            example["images"] = images_list
            if self.ego:
    ...
    def _make_dataset_video(self, info_path, nframes):
        ...
        for cam_name, video_info in all_cam_video_info.items():
            ...
            for video_name, frames in tqdm.tqdm(video_info.items(), desc="Making Nuscenes dataset"):
                # view_video = []
                # text_video = []
                # view_video.append(frames)
                # text_video.append(self.camera_captions[cam_name])
                
                # output_videos.append(view_video)
                # output_text_videos.append(text_video)

                output_videos.append(frames)
                output_text_videos.append(self.camera_captions[cam_name])

            all_cam_output_videos[cam_name] = output_videos
            all_cam_output_text_videos[cam_name] = output_text_videos

        return all_cam_output_videos, all_cam_output_text_videos
