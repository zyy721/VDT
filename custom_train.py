...
def main(args):
    ...
    # model = VDT(depth=12, hidden_size=384, patch_size=2, num_heads=6, input_size=50, num_frames=11)
    ...
    diffusion = create_diffusion(timestep_respacing="", num_frames=nframes, training=True)  # default: 1000 steps, linear noise schedule

    # vae = AutoencoderKL.from_pretrained(f"stabilityai/sd-vae-ft-{args.vae}").to(device)


    # load config
    cfg = Config.fromfile('utils_occworld/OccWorld/config/custom_train_occworld.py')
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_dataset_loader:
            ...
            cond_mask = torch.ones([batch_size, nframes, 1, 1, 1]).to(device)
            cond_mask[:, :nframes_past] = 0
            cond_mask = cond_mask.permute(2, 0, 1, 3, 4)

            with torch.no_grad():
                # Map input images to latent space + normalize latents:
                occ_z, occ_shapes = occ_vae.forward_encoder(input_occs)
                latents, occ_z_mu, occ_z_sigma, occ_logvar = occ_vae.sample_z(occ_z)
                latents = latents.mul_(1.38889)
            ...
            model_kwargs = dict() # Place Holder
            loss_dict = diffusion.training_losses(model, latents, t, model_kwargs=model_kwargs, mask=cond_mask)
